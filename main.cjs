...
const ENTRY_URL = `${APP_URL}/reports/new`;
...
const STARTUP_LOG_PATH = "C:\\tmp\\dailySummary-startup.log";

function writeStartupLog(message) {
  fsSync.appendFileSync(STARTUP_LOG_PATH, `${new Date().toISOString()} ${message}\n`);
}
...
process.on("unhandledRejection", (error) => {
  writeStartupLog(`unhandledRejection: ${error instanceof Error ? error.stack : String(error)}`);
});

writeStartupLog(`main loaded packaged=${app.isPackaged} cwd=${process.cwd()} resources=${process.resourcesPath}`);
...
if (!gotSingleInstanceLock) {
  writeStartupLog("single instance lock failed");
  app.quit();
}
...
app.whenReady()
  .then(async () => {
    ...
    registerIpcHandlers();
    writeStartupLog("before ensureNextServer");
    await ensureNextServer();
    writeStartupLog("after ensureNextServer");
    createTray();
    showPopupWindow();
    scheduleDailyPopup();
  })
  .catch((error) => {
    writeStartupLog(`startup catch: ${error instanceof Error ? error.stack : String(error)}`);
    dialog.showErrorBox(
      `${APP_NAME} 시작 실패`,
